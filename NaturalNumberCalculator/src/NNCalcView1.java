...
public final class NNCalcView1 extends JFrame implements NNCalcView {
	...
	/**
	 * State variable to keep track of which event happened last; needed to prepare
	 * for digit to be added to bottom operand.
	 */
	 ...
	 */
	private final JButton bClear, bSwap, bEnter, bAdd, bSubtract, bMultiply, bDivide, bPower, bRoot;
	 ...
	 * Useful constants.
	 */
	private static final int TEXT_AREA_HEIGHT = 5, TEXT_AREA_WIDTH = 20, DIGIT_BUTTONS = 10,
			MAIN_BUTTON_PANEL_GRID_ROWS = 4, MAIN_BUTTON_PANEL_GRID_COLUMNS = 4, SIDE_BUTTON_PANEL_GRID_ROWS = 3,
	...
	public NNCalcView1() {
		...
		/*
		 * Call the JFrame (superclass) constructor with a String parameter to name the
		 ...
		 * currentState is not a GUI widget per se, but is needed to process digit
		...
		/*
		 * Text areas should wrap lines, and should be read-only; they cannot be edited
		 * because allowing keyboard entry would require checking whether entries are
		 * digits, which we don't want to have to do
		 */
		...
		/*
		 * Initially, the following buttons should be disabled: divide (divisor must not
		 * be 0) and root (root must be at least 2) -- hint: see the JButton method
		 * setEnabled
		 */
		...
		/*
		 * Create scroll panes for the text areas in case number is long enough to
		 * require scrolling
		 */
		...
		/*
		 * Add the buttons to the main button panel, from left to right and top to
		 * bottom
		 */
		...
		/*
		 * Add the buttons to the side button panel, from left to right and top to
		 * bottom
		 */
		...
		/*
		 * Create combined button panel organized using flow layout, which is simple and
		 * does the right thing: sizes of nested panels are natural, not necessarily
		 * equal as with grid layout
		 */
		...
		comButtonPanel.add(mainButtonPanel);
		...
		/*
		 * Add scroll panes and button panel to main window, from left to right and top
		 * to bottom
		 */
		...
		/*
		 * Make sure the main window is appropriately sized, exits this program on
		 * close, and becomes visible to the user
		 */
	...
	public void actionPerformed(ActionEvent event) {
		/*
		 * Set cursor to indicate computation on-going; this matters only if processing
		 * the event might take a noticeable amount of time as seen by the user
		 */
		this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
		/*
		 * Determine which event has occurred that we are being notified of by this
		 * callback; in this case, the source of the event (i.e, the widget calling
		 * actionPerformed) is all we need because only buttons are involved here, so
		 * the event must be a button press; in each case, tell the controller to do
		 * whatever is needed to update the model and to refresh the view
		 */
